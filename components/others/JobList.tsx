  ...
  return (
    <>
      {jobs &&
        jobs.length > 0 &&
        (type === "featured" ? (
          <section className="w-full grid grid-cols-4 gap-6 m:flex m:gap-4 m:overflow-x-scroll m:overflow-y-hidden xl:grid-cols-2">
            {jobs.slice(0, 8).map((job) => (
              <JobCard key={job._id} job={job} />
            ))}
          </section>
        ) : type === "latest" ? (
          <section className="w-full grid grid-cols-2 gap-6 m:grid-cols-1 xl:grid-cols-1">
